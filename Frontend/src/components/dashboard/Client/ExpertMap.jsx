...
const ExpertMap = ({ currentProjectId, category, subCategory, onAssign, onClose }) => {
    ...
    const subCategories = {
        'All': ['All'],
        'Planning': ['All', 'Architect', 'Civil Engineer'],
    ...
    const fetchProfessionals = async () => {
        ...
        try {
            ...
            // Send current coordinates (fallback to map center if precise location is unavailable)
            const searchLat = userLocation?.lat || mapCenter[0];
            const searchLon = userLocation?.lon || mapCenter[1];

            params.append('lat', searchLat);
            params.append('lon', searchLon);
            params.append('radius', 5000); // Increased radius for global/fallback view if precision lost
