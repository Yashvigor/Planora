...
const Settings = () => {
    ...
    const { history, clearHistory, loading } = useActivityLog(userId);
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    setProfileData(prev => ({
                        ...
                        resumeUrl: data.resume_path ? `${import.meta.env.VITE_API_URL}/${data.resume_path}` : null,
                        degreeUrl: data.degree_path ? `${import.meta.env.VITE_API_URL}/${data.degree_path}` : null,
                    }));
                    ...
                    {activeTab === 'profile' && (
                        <div className="glass-card rounded-[2.5rem] shadow-xl border border-[#E3DACD]/80 p-8 md:p-12 relative overflow-hidden bg-white/70 backdrop-blur-xl">
                            ...
                            <form onSubmit={handleProfileSave} className="space-y-6 relative z-10">
                                ...
                                {currentUser?.category !== 'Land Owner' && currentUser?.category !== 'LAND OWNER' && currentUser?.category !== 'Admin' && (
                                    <div className="pt-8 border-t border-[#E3DACD]/50 relative z-10">
                                        <div className="flex items-center space-x-3 mb-6">
                                            <div className="p-2 bg-[#C06842]/10 rounded-lg text-[#C06842]">
                                                <Briefcase size={18} />
                                            </div>
                                            <h3 className="text-lg font-serif font-bold text-[#2A1F1D]">Industry Information</h3>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div className="space-y-2">
                                                <label className="text-xs font-bold text-[#8C7B70] uppercase tracking-wider">Experience (Years)</label>
                                                <input type="number" disabled={!isEditing} value={profileData.experience_years} onChange={(e) => setProfileData({ ...profileData, experience_years: e.target.value })} className={`w-full px-4 py-3.5 border rounded-xl outline-none transition-all text-[#2A1F1D] font-bold shadow-sm ${isEditing ? 'bg-[#FDFCF8] border-[#E3DACD]' : 'bg-[#F9F7F2] border-transparent text-[#8C7B70]'}`} />
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-xs font-bold text-[#8C7B70] uppercase tracking-wider">Specialization</label>
                                                <input type="text" disabled={!isEditing} value={profileData.specialization} onChange={(e) => setProfileData({ ...profileData, specialization: e.target.value })} className={`w-full px-4 py-3.5 border rounded-xl outline-none transition-all text-[#2A1F1D] font-bold shadow-sm ${isEditing ? 'bg-[#FDFCF8] border-[#E3DACD]' : 'bg-[#F9F7F2] border-transparent text-[#8C7B70]'}`} />
                                            </div>
                                            <div className="space-y-2 md:col-span-2">
                                                <label className="text-xs font-bold text-[#8C7B70] uppercase tracking-wider">Portfolio Link</label>
                                                <input type="url" disabled={!isEditing} value={profileData.portfolio_url} onChange={(e) => setProfileData({ ...profileData, portfolio_url: e.target.value })} className={`w-full px-4 py-3.5 border rounded-xl outline-none transition-all text-[#2A1F1D] font-bold shadow-sm ${isEditing ? 'bg-[#FDFCF8] border-[#E3DACD]' : 'bg-[#F9F7F2] border-transparent text-[#8C7B70]'}`} />
                                            </div>
                                            {(profileData.resumeUrl || profileData.degreeUrl) && (
                                                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    {profileData.resumeUrl && (
                                                        <a href={profileData.resumeUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-4 bg-white border border-[#E3DACD] rounded-xl hover:border-[#C06842] transition-all group shadow-sm">
                                                            <div className="p-2 bg-[#C06842]/10 rounded-lg text-[#C06842] group-hover:scale-110 transition-transform"><FileText size={18} /></div>
                                                            <div><p className="text-[10px] font-black text-[#8C7B70] uppercase">Professional Resume</p><p className="text-xs font-bold text-[#2A1F1D]">View Uploaded CV</p></div>
                                                        </a>
                                                    )}
                                                    {profileData.degreeUrl && (
                                                        <a href={profileData.degreeUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-4 bg-white border border-[#E3DACD] rounded-xl hover:border-[#C06842] transition-all group shadow-sm">
                                                            <div className="p-2 bg-blue-50 rounded-lg text-blue-600 group-hover:scale-110 transition-transform"><Award size={18} /></div>
                                                            <div><p className="text-[10px] font-black text-[#8C7B70] uppercase">Academic Degree</p><p className="text-xs font-bold text-[#2A1F1D]">View Verification Document</p></div>
                                                        </a>
                                                    )}
                                                </div>
                                            )}
                        ...
                        <div className="glass-card rounded-[2.5rem] shadow-sm border border-[#E3DACD]/50 p-10 relative overflow-hidden">
                            ...
                            <div className="space-y-4 relative z-10">
                                {history.length === 0 ? (
                                    ...
                                    history.map((item) => (
                                        <div key={item.log_id || item.id} className="glass-card p-4 rounded-xl border border-[#E3DACD]/30 flex items-start gap-4 hover:bg-[#F9F7F2] transition-colors">
                                            <div className="p-2 bg-[#F9F7F2] rounded-lg mt-1 border border-[#E3DACD]/50 text-[#C06842]">
                                                {item.action.includes('Profile') ? <User size={16} /> : <FileText size={16} />}
                                            </div>
                                            <div className="flex-1">
