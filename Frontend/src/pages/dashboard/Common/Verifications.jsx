                                ...
                                <div className="grid grid-cols-2 gap-6 mt-4 pl-1">
                                    <div>
                                        ...
                                        <p className="text-sm font-bold text-[#5D4037] flex items-center gap-2 bg-[#F9F7F2] px-3 py-1.5 rounded-lg border border-[#E3DACD]/50 w-fit">
                                            ...
                                            {verification.sub_category || verification.category}
                                        </p>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        <p className="text-[10px] text-[#8C7B70] uppercase tracking-wider font-bold mb-1">Documents</p>
                                        <div className="flex gap-2 flex-wrap">
                                            {verification.resume_path ? (
                                                <a href={`${import.meta.env.VITE_API_URL}/${verification.resume_path}`} target="_blank" rel="noopener noreferrer"
                                                    className="flex items-center gap-1 px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-100 rounded-lg text-[10px] font-bold uppercase tracking-wider hover:bg-blue-100">
                                                    <FileText size={11} /> Resume
                                                </a>
                                            ) : <span className="text-[10px] text-[#B8AFA5] italic">No Resume</span>}
                                            {verification.degree_path ? (
                                                <a href={`${import.meta.env.VITE_API_URL}/${verification.degree_path}`} target="_blank" rel="noopener noreferrer"
                                                    className="flex items-center gap-1 px-2.5 py-1 bg-amber-50 text-amber-700 border border-amber-100 rounded-lg text-[10px] font-bold uppercase tracking-wider hover:bg-amber-100">
                                                    <FileText size={11} /> Degree
                                                </a>
                                            ) : <span className="text-[10px] text-[#B8AFA5] italic">No Credentials</span>}
                                        </div>
                                    </div>
                                </div>
                            </div>
