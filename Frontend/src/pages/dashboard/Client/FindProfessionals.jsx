...
import { MapPin, Star, Filter, Heart, Search, User, FileText, X, Briefcase, Plus, Clock } from 'lucide-react';
...
const FindProfessionals = () => {
    ...
    const [invitedPros, setInvitedPros] = useState([]);
    ...
    const fetchAssignments = async () => {
        ...
        try {
            ...
            if (res.ok) {
                const data = await res.json();
                setAssignedUserIds(new Set(data.map(member => member.user_id)));
                setInvitedPros(data.filter(member => member.status === 'Pending'));
            } else {
    ...
    useEffect(() => {
        fetchAssignments();
        const poll = setInterval(fetchAssignments, 30000);
    ...
    // Load on role filter, location, or initial mount change
    useEffect(() => {
        fetchPros(
            userLocation?.lat ?? null,
            userLocation?.lon ?? null,
            professionals.length > 0 // silent if we already have data
        );
        const poll = setInterval(() => {
            fetchPros(userLocation?.lat ?? null, userLocation?.lon ?? null, true);
        }, 30000);
        return () => clearInterval(poll);
    }, [selectedRole, userLocation]);

    // Initial sequence to get location + first load
    useEffect(() => {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
        ...
        // First fetch with no location (initial immediate results)
        fetchPros(null, null, false);
    }, []);
    ...
    const handleInvite = async (e, pro) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setAssignedUserIds(prev => new Set(prev).add(pro.user_id || pro.id));
    ...
    const filteredPros = (selectedRole === 'Invited' ? invitedPros : professionals).filter(pro => {
        // If in Invited tab, we only show those in invitedPros state
        if (selectedRole === 'Invited') {
            if (!searchQuery) return true;
            const query = searchQuery.toLowerCase();
            return pro.name?.toLowerCase().includes(query) ||
                   pro.sub_category?.toLowerCase().includes(query) ||
                   pro.category?.toLowerCase().includes(query);
        }

        // Exclude if already assigned to selected project (Pending or Accepted)
        ...
        // Hide Contractors if current user is also a Professional (e.g. Architect finding another Architect is fine, but we keep this simple for now)
        // Landowners SHOULD see contractors.
        if (currentUserCategory !== 'LAND OWNER' && currentUserCategory !== 'LANDOWNER' && currentUserCategory !== 'LAND_OWNER') {
            if (proCat === 'CONTRACTOR' || proSubCat === 'CONTRACTOR') return false;
        }
            ...
            <div className="mb-8 space-y-6">
                <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 bg-white/50 backdrop-blur-md p-6 rounded-3xl border border-[#E3DACD]/50 shadow-sm">
                    <div className="flex items-center gap-5">
                        <div className="p-3 bg-gradient-to-br from-[#E68A2E] to-[#C06842] text-white rounded-2xl shadow-lg shadow-[#C06842]/30">
                            <Briefcase size={28} />
                        </div>
                        <div>
                            <h1 className="text-3xl font-serif font-black text-[#2A1F1D] tracking-tight leading-tight">Professional Discovery</h1>
                            <p className="text-xs font-bold text-[#A65D3B] uppercase tracking-[0.2em] mt-1 opacity-80">Premium Experts • Local Teams</p>
                        </div>
                    </div>

                    <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
                        {/* Project Selector */}
                        <div className="relative min-w-[220px] group flex items-center">
                            <select
                                value={selectedProjectId}
                                onChange={(e) => setSelectedProjectId(e.target.value)}
                                className="w-full pl-5 pr-12 py-3.5 bg-white border border-[#E3DACD] rounded-2xl appearance-none focus:outline-none focus:ring-2 focus:ring-[#b96a41]/50 focus:border-[#b96a41] cursor-pointer text-sm font-bold text-[#5D4037] shadow-sm transition-all group-hover:border-[#b96a41]/50"
                            >
                                {projects.length === 0 ? (
                            ...
                            <div className="absolute right-4 top-3.5 flex items-center gap-2">
                                <button 
                                    onClick={() => {
                                        if (navigator.geolocation) {
                                            navigator.geolocation.getCurrentPosition(
                                                (pos) => setUserLocation({ lat: pos.coords.latitude, lon: pos.coords.longitude, source: 'live' }),
                                                () => alert("Location access denied. Please enable GPS for live discovery.")
                                            );
                                        }
                                    }}
                                    className="p-1 hover:bg-[#b96a41]/10 rounded-full text-[#b96a41] transition-colors"
                                    title="Fetch Live Location"
                                >
                                    <MapPin size={16} />
                                </button>
                                <Briefcase size={16} className="text-[#b96a41] opacity-40" />
                            </div>
                        </div>
                                ...
                                <div className="p-7 flex-1 flex flex-col">
                                    ...
                                    <div className="grid grid-cols-2 gap-3 mt-auto pt-4 border-t border-[#E3DACD]/30">
                                        ...
                                        <button
                                            ...
                                            className={`group/invite flex items-center justify-center px-4 py-3 rounded-xl text-sm font-bold transition-all border ${
                                                assignedUserIds.has(pro.user_id)
                                                ? 'bg-amber-50 text-amber-600 border-amber-200 cursor-default'
                                                : 'bg-gradient-to-br from-[#3E2B26] to-[#2A1F1D] text-white hover:from-[#b96a41] hover:to-[#8a4d2f] shadow-md shadow-[#3E2B26]/20 disabled:opacity-50 disabled:cursor-not-allowed border-[#3E2B26] hover:border-[#b96a41]'
                                            }`}
                                        >
                                            {assignedUserIds.has(pro.user_id) ? (
