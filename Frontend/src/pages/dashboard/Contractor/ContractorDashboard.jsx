    ...
    return (
        <Card className="mb-12" variant="glass">
            <SectionHeader 
                ...
                action={<div className="text-[9px] font-black uppercase text-[#C06842] bg-[#C06842]/5 px-6 py-2.5 rounded-full border border-[#C06842]/10 tracking-[0.2em]">Phase Management</div>}
            />
                    ...
                    return (
                        <div key={phase.id} className={`p-8 rounded-[2.5rem] border-2 transition-all duration-500 flex flex-col items-center text-center space-y-6 ${
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#C06842]">{phase.weight}% Completion</p>
                            </div>
                            <Button
                                size="md"
...
const ContractorDashboard = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const projRes = await fetch(`${import.meta.env.VITE_API_URL}/api/professionals/${uid}/projects`, { headers });
            if (projRes.ok) {
                const projData = await projRes.ok ? await projRes.json() : [];
                setInvitations(projData.filter(p => p.assignment_status === 'Pending'));
                setProjects(projData.filter(p => p.assignment_status === 'Accepted' || !p.assignment_status));
            }
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-24 pb-32 pt-16 font-sans text-[#2A1F1D]">
            ...
            <Card variant="dark" className="relative p-16 overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-16">
                    <div className="flex items-center space-x-12">
                        <div className="relative group shrink-0">
                            <div className="absolute -inset-1.5 bg-gradient-to-r from-[#C06842] to-[#E68A2E] rounded-[2.2rem] blur opacity-40 group-hover:opacity-75 transition duration-1000"></div>
                            <div className="relative w-36 h-36 rounded-[2rem] p-1.5 bg-[#2A1F1D] border border-white/10 overflow-hidden">
                                <img src={`https://ui-avatars.com/api/?name=${currentUser?.name}&background=C06842&color=fff&size=256`} alt="Contractor" className="w-full h-full rounded-[1.6rem] object-cover shadow-2xl" />
                            </div>
                        </div>
                        <div className="space-y-4">
                            <div className="flex items-center gap-3">
                                <span className="h-[1px] w-8 bg-[#C06842]" />
                                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#C06842]">Overview</span>
                            </div>
                            <h1 className="text-6xl font-serif font-black tracking-tight leading-none mb-2">{currentUser?.name}</h1>
                            <div className="flex flex-wrap items-center gap-6 text-[11px] font-black uppercase tracking-[0.2em] text-[#8C7B70]">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-6 w-full lg:max-w-xl">
                        {[
                            ...
                            return (
                                <div key={idx} className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem] flex flex-col items-center justify-center group hover:bg-white/10 transition-all">
                                    <div className={`text-[10px] font-black uppercase tracking-widest mb-3 flex items-center gap-2 text-[#8C7B70] group-hover:text-white transition-colors`}>
                                        <Icon size={14} /> {stat.label}
                                    </div>
                                    <p className={`text-4xl font-serif font-black ${stat.color || 'text-[#C06842]'}`}>{stat.value}</p>
                                </div>
                            );
                        })}
                    </div>
            ...
            {invitations.length > 0 && (
                <Card className="bg-[#C06842]/5 border-[#C06842]/20 flex flex-col md:flex-row items-center justify-between gap-8 py-10">
                    <div className="flex items-center gap-6 text-left">
                        <div className="w-16 h-16 bg-[#C06842] text-white rounded-[1.5rem] flex items-center justify-center shadow-2xl relative shrink-0">
                            <Briefcase size={32} />
                            <div className="absolute -top-1 -right-1 w-5 h-5 bg-white text-[#C06842] rounded-full flex items-center justify-center text-[10px] font-black">{invitations.length}</div>
                        </div>
                        <div>
            ...
            {/* Strategic Projects Feed */}
            <div className="space-y-32">
                {activeProjectPipelines.length === 0 ? (
                    <div className="py-48 text-center bg-white/40 border border-dashed border-[#E3DACD] rounded-[5rem] backdrop-blur-sm relative group">
                        ...
                        <h2 className="text-4xl font-serif font-black text-[#2A1F1D] mb-4 tracking-tight text-center">No Active Projects</h2>
                        <p className="text-[#8C7B70] max-w-sm mx-auto font-medium leading-relaxed text-center">You have no active projects currently.</p>
                    </div>
                ) : (
                    activeProjectPipelines.map(project => {
                                ...
                                <div className="flex flex-col md:flex-row justify-between items-end gap-10 px-8">
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-4">
                                            <span className="w-14 h-14 bg-[#2A1F1D] text-[#C06842] rounded-2.5xl flex items-center justify-center shadow-xl"><Layers size={28} /></span>
                                            <div>
                                                <div className="flex items-center gap-4 mb-2">
                                                    <h2 className="text-5xl font-serif font-black text-[#2A1F1D] tracking-tight uppercase leading-none">{project.name}</h2>
                                                    <span className="text-[9px] font-black uppercase text-[#C06842] bg-[#C06842]/5 border border-[#C06842]/10 px-4 py-2 rounded-full tracking-[0.22em]">{project.assigned_role || 'Project Lead'}</span>
                                                </div>
                                    ...
                                    <div className="w-full max-w-md space-y-3">
                                        <div className="flex justify-between items-end">
                                            <span className="text-[10px] font-black uppercase text-[#8C7B70] tracking-[0.2em]">Project Progress</span>
                                        ...
                                        <Card variant="flat" className="backdrop-blur-md">
                                            ...
                                            <div className="space-y-4">
                                                {[
                                                    ...
                                                    const Icon = res.icon;
                                                    return (
                                                        <button key={idx} className="w-full p-6 bg-white rounded-3xl border border-[#E3DACD]/60 hover:border-[#C06842] text-left transition-all group flex items-center justify-between active:scale-95">
                                                            ...
                                                            <ArrowUpRight size={14} className="text-[#E3DACD] group-hover:text-[#C06842] transition-colors" />
                                                        </button>
                                                    );
                                                })}
                                            </div>
